...
public class InvoiceService {
	...
	@Transactional(readOnly = true) public List<InvoiceResponse> getMine() { return invoiceRepository.findByStudentId(currentStudent().getId()).stream().map(this::toResponse).toList(); }
	...
	private Student currentStudent() { return studentRepository.findByEmailIgnoreCase(currentUsername()).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Student profile not found")); }
	private String currentUsername() { Authentication authentication = SecurityContextHolder.getContext().getAuthentication(); if (authentication == null || authentication.getName() == null) throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Authentication required"); return authentication.getName(); }
